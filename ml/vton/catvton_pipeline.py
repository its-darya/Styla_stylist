...
class CatVTONPipeline:
    ...
    def try_on_garment(self, avatar_image_path: str, garment_image_path: str, category: str) -> str:
        ...
        try:
            # Fix paths if they are URLs
            local_garment_path = garment_image_path
            if garment_image_path.startswith("http") or garment_image_path.startswith("/data"):
    ...
    def try_on_outfit(self, avatar_id: str, outfit_items: list, output_dir: str) -> str:
        ...
        if not os.path.exists(avatar_path):
            ...
            with open(avatar_path, "wb") as f:
                f.write(requests.get(f"https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=400&q=80").content)
        ...
        if tops:
            top_garment = tops[0]
            current_avatar = self.try_on_garment(current_avatar, top_garment["imageUrl"], "top")
            
        if bottoms:
            bottom_garment = bottoms[0]
            current_avatar = self.try_on_garment(current_avatar, bottom_garment["imageUrl"], "bottom")
            
        os.makedirs(output_dir, exist_ok=True)
        final_out_path = os.path.join(output_dir, f"{avatar_id}_final_{uuid.uuid4().hex[:6]}.png")
        
        if current_avatar != avatar_path and os.path.exists(current_avatar):
